...
class BasicLayout extends Component {
    ...
    renderHeader = () => {
        ...
        const noticeMenu = isEmpty(notices) ? (
            <div className={`${prefixCls}-noticeEmpty`}>{intl.formatMessage({ id: 'basicLayout_readall_notice' })}</div>
        ) : (
            map(notices, (notice) => (
                <div
                    key={notice.id}
            ...
            <Breadcrumb className={`${prefixCls}-breadcrumb`}>
                {map(breadcrumbData, (item, idx) =>
                    idx === breadcrumbData.length - 1 ? (
                        <Breadcrumb.Item key={item.href}>{intl.formatMessage({ id: item.text })}</Breadcrumb.Item>
                    ) : (
    ...
    renderFooter = () => <div className={`${this.props.prefixCls}-footer`}>Copyright © 2018</div>;
    ...
    renderNotification = () => {
        ...
        return <Notification title={title} content={content} onDismiss={resetNotification} />;
    };
